...
for(int iter=0; iter<iter_num; iter++){
    // cout << "training file " << iter << ", total seq_num: " << seq.size() << ", single seq_size: " << seq[0].size() << endl;
    // for every iteration, declare params needed for update [a, b, pi] at the end of the iteration.
    double gamma_obs[MAX_OBSERV][MAX_STATE];
    double gamma_accum[MAX_STATE];
    double gamma_last[MAX_STATE];
    double gamma_one[MAX_STATE];
    double epsilon_accum[MAX_STATE][MAX_STATE];
    // the accumulated params need to be initialize as zeros
    ...
    // start running through all the seqs in the seq_file.
    for(int seq_idx=0; seq_idx<seq.size(); seq_idx++){
        line = seq[seq_idx];
        // init the first column of alpha (t=0)
        for(int i=0; i<N; i++){
            alpha[0][i] = PI[i]*B[line[0]-'A'][i];
        }
        // forward algorithm (t=1~T)
        ...
        for(int t=0;t<line.size();t++){
            ...
            for(int i=0;i<N;i++){
                ...
                gamma_obs[line[t]-'A'][i] += gamma[t][i];///(line.size()-1);
            }
        }
        // sum up epsilon
        for(int t=0;t<line.size()-1;t++){
            ...
            for(int i=0;i<N;i++){
                for(int j=0;j<N;j++){
                    divider += alpha[t][i]*A[i][j]*B[o_tplus1][j]*beta[t+1][j];
                }
            }
            for(int i=0;i<N;i++){
                for(int j=0;j<N;j++){
                    epsilon[t][i][j] = alpha[t][i]*A[i][j]*B[o_tplus1][j]*beta[t+1][j]/divider;
                }
            }
        }
        ...
        for(int t=0;t<line.size();t++){
            for(int i=0;i<N;i++){
                ...
                if(t==line.size()-1){
                    gamma_last[i] += gamma[t][i];
                    continue;
                }
                gamma_accum[i] += gamma[t][i];///(line.size()-1);
                for(int j=0;j<N;j++){
                    epsilon_accum[i][j] += epsilon[t][i][j];///(line.size()-1);
                }
            }
        }
    }
    ...
    for(int i=0;i<N;i++){
        for(int k=0;k<K;k++){
            ...
            // g_sum = gamma_accum[i];
            // }
            B[k][i] = gamma_obs[k][i]/(gamma_accum[i]+gamma_last[i]);
        }
    }
}
